...
uniform float u_meteors; // whether stars throws a shooting star on an onset
uniform float u_wake;    // the aurora's charge: fast to rise, very slow to fall
uniform float u_heat;    // how hot the air is. Drives the shimmer over the far
                         // ground and the haze that bleaches the horizon — the
                         // two things that separate a hot landscape from a
                         // temperate one drawn in warm colours
uniform float u_flower;  // whether whole open blooms turn among the falling
                         // petals. Its own parameter rather than a reading of
                         // u_leaf, so no crossfade between two flowerless moods
                         // can pass through the window and put blossom in a
                         // dead wood on the way (see mPetals)
...
// A STAND OF TREES, enumerated. Discrete trunks at hashed positions, each with
// its own width, lean, sinuosity, taper, root flare and colour — in three
// depth layers that pass at their own rates as the flow clock advances.
//
// What was here before was a THRESHOLD ON A NOISE FIELD: fbm(vec2(x, 4.7)),
// with a constant second argument. That is a one-dimensional function of x, so
// every "trunk" was a vertical band of uniform width running the full height of
// the frame, with hard parallel edges and no top, bottom or middle. The owner's
// read of it is exact — "banded stripes… not quite like old VHS tape bands, but
// not quite unlike it either" — and the reason it survived four passes is that
// a dark green wood hides it: the bands are dim shapes in a mist and pass as
// trunks. Barren picks them out in their own colour and the illusion collapses.
// The owner's call, taken: fix the trunks themselves rather than the mood.
//
// The differences that make it wood rather than corduroy, in the order they
// matter:
//
//   POSITION AND WIDTH ARE PER TRUNK. A lattice with a hashed offset inside
//   each cell, so the spacing is uneven and no two trunks are the same size.
//   Two cells are checked, not one — a trunk sits off its cell centre and its
//   branches reach past the boundary.
//
//   THEY ARE NOT STRAIGHT. A fixed lean per trunk plus a slow linear noise down
//   its length. A tree is sinuous; a rectangle is not, and nothing else about a
//   vertical dark shape says "grown" so cheaply.
//
//   THEY TAPER AND THEY FLARE. Narrower toward the canopy, wider where they
//   meet the ground. Both small — a trunk that tapers hard is a spike.
//
//   THE SHADING IS A REAL NORMAL. The old cylinder term differenced the noise
//   field a little to one side, which is a gradient of the MASK and says only
//   where an edge is. Here the distance from the centreline, signed and divided
//   by the half-width, IS the surface normal across the trunk: -1 at the left
//   silhouette, 0 at the centre, +1 at the right. Light from the upper left
//   then lands where it should, and the falloff is the cylinder's own.
//
//   AND THE FAR STANDS ARE EATEN FROM THE TOP DOWN. Mist is thickest where the
//   canopy is. That, plus the parallax, is the depth.
//
// Branches are drawn only where bark says the trunks are the subject (barren
...
float mColumns(vec2 uv, float t, float flow, float bark, float solid,
  ...
  lit = 0.0;
  float v = 0.0;
  // Height in the frame, 0 at the bottom of the aperture and 1 at the top.
  // Taper, flare and the canopy fade all want a coordinate that does not move
  // with the lens.
  float hgt = clamp(uv.y + 0.5, 0.0, 1.0);
  ...
  for (int L = 0; L < 3; L++) {
    ...
    // FEWER AND THEREFORE THICKER, and this is the whole of "grass blades in
    // Honey I Shrunk the Kids".
    //
    // At 8.5 the far layer put fourteen columns across the aperture and the
    // near one six. Fourteen uprights across a frame is a reed bed at any
    // width, and the half-width that fits between them (0.085-0.20 of a cell)
    // came out at ten to twenty-five pixels — a blade. What says TRUNK is not
    ...
    float xs = uv.x * fx + fl * 17.3 + t * 0.010 * (1.0 + fl)
             + flow * (0.10 + fl * 0.26);
    float cell = floor(xs);
    float off = fract(xs) - 0.5;
    float bestM = 0.0;
    for (int k = 0; k < 2; k++) {
      float ci = cell + (k == 0 ? 0.0 : (off < 0.0 ? -1.0 : 1.0));
      float f = xs - ci - 0.5;                  // offset from this cell's centre
      vec2 h1 = ch2(vec2(ci, fl * 7.0 + 1.0));
      ...
      // Wood is stiff; holding a line is most of what makes it read as wood.
      // Barren keeps more of the bend, because a dead tree does lean and the
      // owner liked it there: "I like that the trunks are...more bent here?"
      float lean = (h2.x - 0.5) * mix(0.16, 0.28, bark);
      float bendx = lean * (hgt - 0.35)
                  + (lnoise(vec2(hgt * 1.5 + ci * 5.7, fl * 3.3)) - 0.5)
                    * mix(0.075, 0.14, bark);
      ...
      // NEARLY PARALLEL-SIDED. Losing a quarter of the width over the visible
      // height is a spear of grass. A trunk of any age barely narrows over the
      // few metres you can actually see, and the flare at the root is what says
      // it is rooted rather than stuck in.
      float taper = 1.0 - hgt * 0.09 + smoothstep(0.085, 0.0, hgt) * 0.22;
      ...
      // reaches ZERO at the top of the frame — so every far trunk faded out to
      // nothing before it left the picture, and the owner could not tell
      // whether they were tapering away or lost in mist: "the tops of some of
      // these non-trees in the back taper off into nothing (or are shrouded in
      // mist, impossible to tell which)". Neither, as it turned out. They were
      // being erased. A tree leaves the top of the frame; aerial perspective is
      // a loss of CONTRAST, not of the object, so the far stand now keeps its
      ...
      float m = max(cover, br);
      if (m > bestM) {
        bestM = m;
        if (m > 0.02) {
          // Sampled off the cell rather than off a low-frequency field: a trunk
          // is one piece of wood, so its colour must be constant along its
          // whole length and unrelated to its neighbour's.
          tint = fract(h2.y * 3.17 + fl * 0.31);
          lit = shade;
        }
      }
    }
    // Nearer stands are in front. Iterating far to near and letting a layer
    // with real coverage claim the surface properties IS the depth sort.
    if (bestM > 0.02) v = max(v, bestM);
  }
  return v;
}
...
float mDrips(vec2 uv, float t, float w, float slant, float drive, float weather, float kick, out float splash) {
  ...
  // CONSTANT, AND IT HAS TO BE. This was mix(3.2, 2.9, w) — the fall rate as a
  // function of the motif's own weight — and w morphs when the mood changes.
  // rate multiplies t below, so the moment w moves, the ELAPSED phase is
  // rescaled: t is tens of units by the time anyone switches mood, so a swing
  // of 0.09 in the rate slides the accumulated phase by three whole cycles.
  // Downwards if the weight rises, and BACKWARDS if it falls — which is the
  // owner's report exactly: "switching to these, even from each other,
  // sometimes causes the rain to fall briefly upward", and sunshower's weight
  // (0.66) is below rain's (0.95), so that direction rewinds.
  //
  // This is §13, the engine's oldest law, in a place nobody had looked: the
  // rule is usually quoted about live audio features, but a MORPHING THEME
  // PARAMETER is a live value too, and it moves much further than a feature
  // does. Any quantity that multiplies a clock has to be a constant or a
  // per-cell hash — never a uniform that a crossfade can change.
  float speed = 3.0;
  ...
  // enormous.
  // A second cut, from the same note: at 0.22 the dense end still drew a streak
  // a quarter of the frame tall, and rain that size in front of a cloud deck is
  // a shift of LOCATION — you are standing under a downpour rather than looking
  // at a landscape that has weather in it. The owner wants rain and sunshower to
  // read as the same place in two weathers, and scope is most of what says so.
  float tail = mix(0.04, 0.15, w);   // and are short
  ...
  // The streak's width is measured in LANE fractions (see thin, below), so
  // raising the count narrows every streak in screen space at the same time as
  // it multiplies them. That is exactly what distance does to rain, and it is
  // why the shrink cannot be done by shortening alone: a short fat streak is a
  // near drop that got cut off, not a far one.
  // CONSTANT for the same reason, and it fixes a second artifact. lanes decides
  // which lane a pixel is in (col = floor(lx * lanes)), and col seeds the hash
  // that gives the lane its rate, its phase and whether it is raining at all.
  // Morphing lanes therefore slides every lane boundary across the screen while
  // re-rolling every lane's identity underneath it, so drips slide sideways and
  // change into different drips mid-crossfade. Density is what the weight is
  // for and duty above already carries it, quadratically.
  const float lanes = 42.0;
  ...
  // Screen-space, unlike everything else here, so it has to be shrunk by hand
  // to match the finer rain — a crown sized for the old streaks would be a
  // splash wider than the drop that made it is long.
  float ring = age * (0.040 + kick * 0.024);
  float crown = smoothstep(0.013, 0.0, abs(length(rel) - ring));
...
// ONE CRYSTAL, GROWN OUTWARD FROM A SEED — and the change from what came
// before is that there is no lattice of ferns any more.
//
// Three passes at this were built on a periodic grid of trunks with branches
// hung off them, and every one of them read as a repeating motif: "a bunch of
// centipedes or fish bones". That is not a tuning failure, it is what a lattice
// IS. A grid repeats; frost does not. No amount of jittering the spacing, the
// lean or the length changes the fact that the structure has a period, and the
// eye finds a period faster than it finds anything else in a picture.
//
// Real frost nucleates at scattered points and grows radially away from each
// one until it runs into its neighbour. So that is what this is: a jittered
// field of seeds, each with its own orientation, its own six arms of its own
// lengths, its own start time and its own rate, expanding into whatever room it
// finds. Nothing about it is periodic, the crystals collide at every angle, and
// the sizes differ because the start times do.
//
// The six-fold fold costs no trigonometry. The arm a point belongs to is
// whichever of three axes has the largest projection, and the sign of that
// projection says which end — six dot products and two comparisons, giving
// (along, across) in the arm's own frame. Everything after that is the sheared
// branch lattice the previous version already used, which was the one part of
// it that was right: a branch leaving its parent at 60 degrees travels 0.58
// along for every 1 across, so u - s * lean finds the parent with a
// subtraction and no search.
//
// GROWTH IS GEOMETRY, NOT A THRESHOLD. The old version drew a static field and
// swept a level set down through it — which fades a picture in, and is why it
// "forms quickly and then stays stable" no matter how slowly the level moves.
// Here the crystal's front is a PATH LENGTH from the seed. An arm is drawn only
// as far as the front has reached; a branch does not exist until the front
// passes its root, and then extends at its own rate from that moment; a
// sub-branch waits for its branch. So the thing on screen genuinely grows
// outward, forks as it goes, and feathers at the ends — and it does it in the
// order a crystal does, because the front is measured along the crystal rather
// than across the picture.
float crystal(vec2 rel, vec2 id, float front, float armCap) {
  float r2 = dot(rel, rel);
  // Cheap reject before anything is computed. Most seeds, most of the time.
  if (r2 > (front + 0.03) * (front + 0.03)) return 0.0;

  // Each crystal's lattice is turned its own way. A normalized hashed vector is
  // the rotation and costs one square root, where a hashed angle would cost two
  // transcendentals — and there are up to nine of these per fragment.
  vec2 cs = normalize(ch2(id + 3.7) * 2.0 - 1.0 + vec2(1e-4, 0.0));
  vec2 p = vec2(rel.x * cs.x + rel.y * cs.y, rel.y * cs.x - rel.x * cs.y);

  const vec2 A0 = vec2(1.0, 0.0),      B0 = vec2(0.0, 1.0);
  const vec2 A1 = vec2(0.5, 0.86603),  B1 = vec2(-0.86603, 0.5);
  const vec2 A2 = vec2(-0.5, 0.86603), B2 = vec2(-0.86603, -0.5);
  float d0 = dot(p, A0), d1 = dot(p, A1), d2 = dot(p, A2);
  float along = abs(d0), acr = dot(p, B0), sgn = d0 < 0.0 ? -1.0 : 1.0, arm = 0.0;
  if (abs(d1) > along) { along = abs(d1); acr = dot(p, B1); sgn = d1 < 0.0 ? -1.0 : 1.0; arm = 1.0; }
  if (abs(d2) > along) { along = abs(d2); acr = dot(p, B2); sgn = d2 < 0.0 ? -1.0 : 1.0; arm = 2.0; }
  ...
  // BRANCHES — but only for fragments that could be ON one. The arms are thin
  // and a crystal is mostly the gaps between them, so about half of every disc
  // is further from the nearest arm than the longest branch can reach. Two
  // guards, one per generation, and they are worth more than any arithmetic in
  // here: this whole block is skipped for those fragments, and the sub-branch
  // block is skipped again for the ones that are not on a branch either.
  float sq = abs(across);
  float branch = 0.0;
  float sub = 0.0;
  ...
  if (sq < REACH1) {
    ...
    // makes every branch congruent, which is the arrowhead read.
    ...
    float len1 = min(want1, have1)
               ...
               * step(0.0, bu) * step(bu, armMax);
    // Guarded rather than clamped: at len1 = 0 the division would hand back 1 on
    ...
    // SUB-BRANCHES, in the branch's own frame: how far out along it we are, and
    // how far off it. Same eight lines one level down, which is the whole reason
    // the fold is worth doing.
    if (tap1 > 0.001) {
      float acrossP = bf / P1;
      float sp = abs(acrossP);
      float lean2 = 0.58 + (lnoise(vec2(sq * 22.0 + bi * 1.3, 29.0)) - 0.5) * 0.66;
      float side2 = acrossP < 0.0 ? 0.5 : 0.0;
      float cu = sq - sp * lean2;
      const float P2 = 80.0;
      float cx = cu * P2 + bi * 2.3 + side2;
      float ci = floor(cx);
      float cf = fract(cx) - 0.5;
      float r2b = ch1(vec2(ci, bi * 0.61 + armId + 4.0));
      float want2 = 0.005 + 0.014 * r2b;
      // Path length to this sub-branch's root: out along the arm, then out along
      // the branch. Growth propagates through the crystal, not through the plane.
  ...
  return max(max(trunk, branch), sub);
}

// SEED SPACING, AND WHY THE NEIGHBOURHOOD IS FOUR CELLS RATHER THAN NINE.
//
// The obvious construction is a 3x3 sweep around the fragment's own cell, which
// lets a crystal reach 1.15 spacings and overlap generously. It is also three
// times the work, and the early-outs that look like they pay for it do not: a
// software rasteriser runs a dynamic loop over a whole SIMD group at once, and
// lanes in neighbouring cells disagree about when to leave, so the body is
// executed with masking rather than skipped. Nine iterations means nine
// iterations. Measured, that put ice at 1.6x the cheapest mood.
//
// Four cells — the ones whose centres BRACKET the fragment — cost less than
// half and bound the reach at 0.65 spacings, for a reason worth writing down:
// a cell outside the bracketing set has its centre at least one spacing away
// along some axis, and the jitter can carry its seed at most 0.35 of a spacing
...
float mFrost(vec2 q, float grow) {
  ...
  for (int j = 0; j <= 1; j++) {
    for (int i = 0; i <= 1; i++) {
      vec2 cid = b + vec2(float(i), float(j));

      // WHEN THIS ONE STARTED, AND HOW FAST IT GOES. Both hashed, and the
      // spread of start times is most of why the pane keeps changing for
      // minutes rather than arriving all at once: a crystal that nucleates late
      // is still a small one when its neighbours are finished, and the size
      // difference is the most legible thing in the references.
      float delay = ch1(cid + 2.5) * 26.0;
      if (grow <= delay) continue;
      float rate = 0.55 + ch1(cid * 1.3 + 7.0) * 0.95;
      float phase = (grow - delay) / 15.0;
      // Saturating, so a long session settles rather than running past itself.
      // The arithmetic, because "several minutes" is a requirement and not a
      // feel: the flow clock runs at roughly 0.2 per second under moderate
      // playing, so grow reaches 26 (the last seed nucleating) at about two
      // minutes, and the earliest crystals are near their final size at three
      // to four. Nothing is finished before then and nothing keeps creeping
      // long after.
      float R = FROST_R * (1.0 - exp(-phase * rate));
      vec2 seed = (cid + 0.5 + (ch2(cid + 11.0) - 0.5) * 0.70) * FROST_S;
      v = max(v, crystal(q - seed, cid, R, FROST_R));
    }
  }

  // GRANULAR RIME, and the references are half made of it: fine frozen fog with
  // the ferns standing out of it into clear glass. It arrives LATE and in its
  // own regions — rime settles into the corners a crystal has already claimed,
...
float mCrystals(vec2 uv, vec2 vp, float t, float selClock, vec2 lightDir,
  ...
  // WHERE THE LIGHT IS, not whose turn it is — and this is the owner's own
  // diagnosis, which is better than the one it replaces.
  //
  // "Some of the crystals are really close to each other and it makes no sense
  // that one cluster would be illuminated while another one that literally
  // overlaps it would be invisible. So the effect is not one of revealing, but
  // of poofing in and out."
  //
  // Exactly so, and no amount of softening the envelope could have fixed it,
  // because the fault was the MODEL: brightness was a function of which slot a
  // cluster occupied in a round-robin, and a slot is not a place. Two clusters
  // sitting on top of each other could be at opposite ends of their windows, so
  // the picture asserted that a light was falling on one and not on the other
  // an inch away, which is not something light does.
  //
  // A lamp is a POSITION. It wanders the passage on the travel clock, and every
  // cluster is lit by how near it happens to be — so overlapping clusters are
  // lit almost identically, by construction, and can never disagree. What the
  // playing changes is how far the light reaches: a low register leaves a tight
  // pool with one seam in it, and the top of the register floods the wall.
  // "Dark, dank cavern vs. crystal grotto" falls straight out of one radius.
  float lampPh = selClock * 0.7;
  vec2 lampP = vp + vec2(cos(lampPh * 1.9) * 0.62, sin(lampPh * 1.31) * 0.46);
  float lampReach = 0.30 + pitch * 0.95;
  ...
  for (int j = 0; j < CRYSTAL_CLUSTERS; j++) {
    ...
    float e = floor(ph);
    float u = fract(ph);
    // HOW MANY SEAMS ARE AWAKE AT ONCE IS THE REGISTER'S BUSINESS. The window
    // used to be a fixed 0.58-1.0, so the same one-cluster-at-a-time reveal ran
    // whether the room was playing the bottom of the keyboard or the top. The
    // owner's ask is a difference in KIND — "dark, dank cavern vs. crystal
    // grotto" — and one lever cannot do that. Widening the window with the
    // register means the low end shows a single seam guttering in and out of a
    // black passage, and the top end has every cluster alight at once.
    // The slot now governs only the SEAT'S LIFETIME — a cluster has to fade out
    // before its hashed position is re-rolled, or it would jump across the wall
    // in front of you. It is flat for eighty per cent of the cycle, so two
    // clusters that overlap are almost always both at full slot value and the
    // lamp below is what decides their brightness.
    float slot = smoothstep(0.0, 0.10, u) * (1.0 - smoothstep(0.90, 1.0, u));
    // NO EARLY-OUT ON THE NOMINATION, and this is the owner's "they illuminate
    // so fast that there's always a sense they appear and disappear instead of
    // just lighting up and becoming visible".
    //
    // They were appearing and disappearing — literally. Outside its window a
    // cluster was not dim, it was NOT DRAWN, so the geometry blinked into
    // existence with the light. A crystal is a solid object: it is there in the
    // dark, and what the music changes is how much of it you can see. So the
    // cluster is always traced and env now scales the LIGHTING only, over a
    // floor that keeps a silhouette standing in the passage the whole time —
    // "maybe some part of them could be visible in the interim, like a
    // silhouette", which is exactly right.
    //
    // The spatial reject below is what pays for the enumeration and it is
    // untouched; this one only ever skipped clusters that cover this fragment
    // but happen to be between turns, which is precisely the case we now want.
    ...
    // HOW LIT THIS CLUSTER IS: how near it is to the lamp, times whether its
    // seat is currently alive. Distance, not turn order — so two clusters an
    // inch apart get the same light and the poofing cannot happen.
    float dl = length(seat - lampP) / lampReach;
    float env = slot * exp(-dl * dl);
    // Present in the dark whatever the light is doing; see the note above.
    float pres = 0.16 + 0.84 * env;
    ...
    bedBest = max(bedBest, bed * pres * (0.20 + drive * 0.4));

    // THE MASSIF: the coalesced crystal the spears grow OUT of.
    //
    // Depth ordering fixed which spear owns a pixel, and the owner still reads
    // the result as detached — correctly, because ordering the crossings does
    // not remove them. Six prisms radiating from one point cross each other
    // near that point no matter which one wins, and a cluster of thin shapes
    // meeting in mid-air is a bunch of shards however cleanly they are cut.
    //
    // Real quartz does not do that. Spears rise out of a solid mass of crystal
    // that has grown together at the root, and that mass hides every join. So:
    // a faceted body at the seat, drawn NEARER than any spear in its own
    // cluster, which swallows all the roots at once. The owner asked for
    // exactly this — "a crystalline base that goes over all the overlapping
    // crystals, hiding those weird overlaps and base disparities".
    //
    // Faceted, not domed, and that is what lnoise is for: it is piecewise
    // linear, so its gradient is constant inside a lattice cell and kinks at
    ...
    // Which way is out of the wall at this seat. Hoisted out of the spear loop
    // below, because the massif needs it too — the bed lies ALONG the rock, and
    // it cannot know which way that is without this.
    vec2 inwardOf = -vec2(cos(ang), sin(ang));
    vec2 dm = d0 / persp;
    float mr = length(dm);
    vec2 nd = dm / max(mr, 1e-4);
    vec2 fdir = vec2(nd.x * 2.2 + fj, nd.y * 2.2 + e);
    // A NEST, NOT A BOULDER. The polygon fixed the sphere but left a compact
    // convex lump, and a compact convex lump at the root of a spray of prisms
    // reads as one more prism aimed at the viewer — the owner's note exactly:
    // "the bases look like another crystal spear pointed straight at the
    // 'camera', so it isn't really helping."
    //
    // Two changes. The radius gains a sparse spiky term, so the outline throws
    // occasional points instead of closing smoothly — the "vague spiky nest"
    // that was asked for, and what the rim of a geode actually looks like. And
    // the whole mass is FLATTENED AGAINST THE WALL: wide along the rock and
    // shallow out of it, because a bed of crystal spreads over the surface it
    // nucleated on. A mass that is as deep as it is wide is a boulder; one that
    // is three times wider than deep is a crust with things growing out of it.
    float spike = max(0.0, lnoise(fdir * 3.3 + 5.0) - 0.52);
    float mRad = 0.115 * (0.68 + 0.50 * lnoise(fdir) + 1.35 * spike);
    // The cluster's own frame: out of the wall, and along it.
    vec2 outw = -inwardOf;
    vec2 tang = vec2(-outw.y, outw.x);
    vec2 dmw = vec2(dot(dm, tang), dot(dm, outw) * 2.1);
    float mrFlat = length(dmw);
    float mound = 1.0 - smoothstep(mRad * 0.90, mRad, mrFlat);
    if (mound > 0.003) {
      ...
      float mrim = smoothstep(0.86, 1.0, mrFlat / max(mRad, 1e-4));
      ...
      // Present in the dark, lit when nominated — the same law as the spears.
      float mv = mound * (pres * mbody + env * (mglint * mgain + mglint * strike * 0.85));
      // Nearer than every spear this cluster has, so the roots go inside it.
      float mz = dep - 0.05;
      if (mz < bestZ) {
        bestZ = mz; best = mv; faceGlow = env * mound * mglint; tint = qtint;
      }
    }
    ...
    for (int k = 0; k < CRYSTAL_SPEARS; k++) {
      ...
      // A CUBIC, AND A LOWER FLOOR. This was 0.18 + drive*0.5 + pitch*pitch*0.75,
      // which spans about 3:1 from the bottom of the keyboard to the top — a
      // difference in degree, where the owner asked for a difference in kind.
      // Dropping the floor to 0.10 and cubing the register gives better than
      // 20:1: low playing leaves a dim shape in a black passage, and the top of
      // the instrument turns the same seam into a lit grotto. The cube is what
      // keeps the middle of the keyboard from already being bright — a square
      // still delivers a quarter of the light at half the register, and a
      // quarter is plenty to see by.
      float lampGain = 0.10 + drive * 0.40 + pitch * pitch * pitch * 2.4;
      // The diffuse body answers the register too, or the crystals stay equally
      // present as shapes and only their highlights change.
      // SPARKLES, and how many of them is the register's whole business.
      //
      // "Like one face lights up dimly or there's a sparkle. And then with the
      // high register, lots of sparkles and full crystal illumination." A
      // crystal in a beam does not brighten uniformly — it throws points of
      // light off its edges and terminations, and the number of them is what
      // separates a dank cavern from a grotto far more legibly than any overall
      // gain does. Segments along the prism, re-hashed a couple of times a
      // second so they twinkle rather than sit; the threshold drops with the
      // register, so the bottom of the keyboard lights one or two and the top
      // sets the whole seam glittering.
      float seg = floor(uu * 6.0) + fk * 3.1 + fj * 11.0;
      float spk = ch1(vec2(seg, band + floor(t * 2.3)));
      float sparkle = step(0.93 - pitch * 0.42, spk) * edge
                    * (0.20 + pitch * 1.5 + strike * 0.9);
      // PRESENT ALWAYS, LIT SOMETIMES: the mass is there in the dark at a
      // sixth of its brightness and the nomination decides the rest. Without
      // this the object itself was blinking, not the light on it.
      float v = pres * body * (0.28 + pitch * 1.1)
              + env * (glint * lampGain + glint * strike * 0.85 + sparkle);
...
float mLava(vec2 uv, float t, float flow, float drive, float kick,
  ...
  // FEWER, AND THAT IS THE DRAMA. At 2.6 + run*5.0 the cone carried a dozen or
  // more channels — a continuous curtain of orange from rim to base, which is
  // what made the mountain read as melting rather than erupting, and is most of
  // why the owner finds it "not terribly compelling". Their own observation is
  // the specification: "I'm not saying there need to be as many rivulets as
  // there are in the photos. Even in photos where there are just a few, it's
  // breathtaking." A few bright threads on a black cone is a stronger image
  // than a wall of them, because then each one is a thing you can follow.
  vec2 q = vec2(across * (1.5 + run * 2.4) + bend * 1.4,
...
float moteShape(vec2 q, float leaf, float flower, out float rib, out float eye) {
  rib = 0.0;
  eye = 0.0;
  // A WHOLE BLOSSOM, for the share of motes that are flowers rather than
  // petals. Five rounded lobes about a centre, with a bright eye in the middle.
  //
  // cos(5 * atan) is safe across the atan wrap, which the engine otherwise
  // forbids: the discontinuity at +-pi lands on a whole number of the cosine's
  // own turns, so the two sides agree. (Feeding atan to fbm would not, and that
  // is the rule this is an exception to rather than a breach of.)
  if (flower > 0.5) {
    float rr = length(q);
    // cos(5 * theta) WITHOUT the angle. The fifth Chebyshev polynomial gives it
    // straight from cos(theta) = q.x / r: T5(c) = 16c^5 - 20c^3 + 5c. Pure
    // arithmetic, where atan followed by cos is two transcendentals — and this
    // sits three deep inside the mote loops, so it would be paid up to nine
    // times a fragment. It also sidesteps the atan wrap entirely rather than
    // relying on five turns landing back where they started.
    float c = q.x / max(rr, 1e-4);
    float c2 = c * c;
    float lobe = 0.76 + 0.24 * (c * (16.0 * c2 * c2 - 20.0 * c2 + 5.0));
    float cover = smoothstep(lobe, lobe * 0.55, rr);
    eye = smoothstep(0.34, 0.06, rr) * cover;
    return cover;
  }
  float s = q.y * 0.5 + 0.5;                       // 0 at the stem, 1 at the tip
  // PETAL: narrow where it joins the flower, broad and ROUNDED at the tip.
  //
  // It used to be a lens closed at both ends by sqrt(4s(1-s)) — which comes to
  // a cusp at each end, and a cusped ellipse is an almond, or, at the size these
  // are drawn, a shard. That is exactly what the owner saw: "they look and fall
  // more like hard shards of something". A blossom petal is the other shape
  // entirely: it leaves the stem narrow, widens fast, and finishes in a
  // semicircular cap. pow(s, 0.42) is the fast widening; sqrt(1 - s*s) is the
  // cap, and its vertical tangent at the tip is what makes the end read as
  // round rather than as a point.
  float petalW = 0.78 * pow(max(s, 0.0), 0.42) * sqrt(max(1.0 - s * s, 0.0));
  ...
  // The edge is soft for a petal and tight for a leaf. A blossom petal is thin
  // enough to pass light and its outline is the first thing to go; a dead leaf
  // is opaque and has a definite edge. Same knob, opposite ends.
  float soft = mix(0.30, 0.62, leaf);
  float cover = smoothstep(halfW, halfW * soft, abs(q.x))
              * smoothstep(1.06, 0.90 + leaf * 0.06, abs(q.y));
  rib = leaf * smoothstep(0.09, 0.0, abs(q.x)) * cover;
  return cover;
}

// THE AIR MOVES, AND THAT IS THE WHOLE OF THIS PASS.
//
// Every previous version gave each mote a sway about its own seat: a bounded
// oscillation around a FIXED x. The net horizontal travel of a petal over its
// whole fall was therefore zero, and no amount of tuning the swing changes
// that — "they just fall straight down" is a statement about displacement, and
// the code had none to give. Leaves were worse than petals because their
// (larger) shared gust was still a swing about a fixed point, so the whole
// layer leaned one way and back rather than anything being carried.
//
// The cure is to stop moving the motes and move the AIR. Both lattice
// coordinates are warped by a wind field before they are floored:
//
//   lane  sheared by a wind that varies with HEIGHT, so a mote sweeps sideways
//         as it descends and its path is a curve rather than a plumb line —
//         and motes at different heights go different ways at once, which is
//         what makes a gust read as a gust and not as a pan.
//
//   y     lifted by an updraft that varies with x, so a mote slows, hangs and
//         rises a little as it crosses a thermal. This is what "floaty" is: a
//         petal does not descend at a constant rate, it dawdles.
//
// A mote's identity is its cell in the WARPED grid, so it is carried by the
// field instead of being displaced within its cell — the travel is unbounded
// and costs no extra neighbours to look at.
//
// Both fields take the flow clock in their PHASE, never in their amplitude.
// An amplitude on a live feature is the retreat fault in its purest form: the
// room quietens, every displacement shrinks, and the whole shower slides back
// toward where it would have been in still air. Loudness buys gustier air by
// making the gusts pass sooner; it never buys a bigger displacement.
float mPetals(vec2 uv, float t, float w, float leaf, float drive, float kick,
              float fall, out float tint) {
  tint = 0.5;
  float v = 0.0;
  // One phase clock for the weather, summing two monotonic clocks.
  float ph = t * 0.55 + fall * 1.35;
  // How hard the air works. Dead leaves in autumn are thrown about; blossom
  // drifts. Fixed per season, for the reason above.
  // HALVED, and the halving is the point. This field is a function of uv, so
  // every mote in a region receives the SAME push at the same instant — which
  // is precisely the owner's diagnosis, and it was right: "they move more or
  // less as a unit, like they're attached to a transparent plane. (They
  // probably are.)" They were. Warping the lattice moves the sheet the motes
  // are printed on; it can never move one leaf differently from its neighbour.
  //
  // So the shared air keeps only enough amplitude to read as weather passing
  // through — a gust really does take a whole stand of trees at once — and the
  // rest of the motion moves into each mote's own orbit, below, where it can
  // differ from the mote beside it.
  float ampX = mix(0.52, 1.15, leaf);   // in cell widths
  float ampY = mix(0.26, 0.15, leaf);   // in row heights: petals hang, leaves drop
  for (int i = 0; i < 3; i++) {
    float fi = float(i);
    ...
    // less of it is mass.
    // NO leaf FACTOR ON THE CLOCK. This was fall * (...) * mix(0.66, 1.0, leaf),
    // and leaf is what a blooming-to-autumn crossfade lerps from 0 to 1 — so
    // the accumulated fall phase was being rescaled by half again during the
    // morph, throwing every mote down the frame going one way and UP it going
    // the other. The same §13 fault as the rain, in the same pass.
    //
    // The difference between blossom drifting and leaves dropping is still
    // there; it just lives in things that oscillate rather than accumulate —
    // the orbit's amplitude, the flutter, the shape's drag — none of which can
    // rewind, because none of them is a phase.
    float y = uv.y * rows + fall * (0.75 + fi * 0.42)
            + updraft * ampY;
    float lane = uv.x * cols + gust * ampX;
    float row = floor(y);
    for (int k = -1; k <= 1; k++) {
      vec2 c = vec2(floor(lane) + float(k), row);
      float on = step(mix(0.90, 0.79, leaf) - w * 0.26, hash(c + fi * 17.0));
      vec2 seed = hash2(c + fi * 3.0);
      float ph2 = seed.x * 6.2831853;
      float rate = 1.0 + hash(c + 9.0) * 2.0;
      // A WHOLE FLOWER, sometimes — and gated on the season's OWN parameter,
      // never on the shape parameter.
      //
      // This was step(leaf, 0.35), which is a trap that only shows up in
      // motion. Autumn sets leaf = 1 and barren leaves it at 0, so crossfading
      // between two moods that have no flowers in them sweeps leaf down through
      // 0.35 while petals is still fading out — and blossom appears, in a dead
      // wood, for about a second. The owner caught it in both directions.
      //
      // The rule is general: a gate on an interpolated parameter fires whenever
      // the PATH between two themes crosses it, whatever the endpoints say. Gate
      // on something whose endpoints are both outside the window and the path
      // cannot cross.
      float flower = step(0.66, hash(c + 41.0)) * step(0.5, u_flower);
      // The flutter is private to each mote and constant in amplitude; the flow
      // clock only makes it quicker. This is the same law as the wind above.
      float flutter = sin(t * rate * 1.9 + fall * 0.9 + ph2)
                    * (0.55 + seed.y * 0.75) * 0.11;
      // THIS MOTE'S OWN PATH THROUGH THE AIR, which is the answer to the plane.
      //
      // The two components are in QUADRATURE — sine against cosine — so the
      // mote traces a loop rather than shuttling along a diagonal. A loop
      // superimposed on a steady descent is a helix seen edge-on, and that is
      // what a leaf coming off a tree actually does: it swings out, stalls,
      // slides back across its own path and drops again. The owner asked for
      // "swirling, not just falling", and swirling is a closed path.
      //
      // Its phase and both its rates are hashed per cell, so no two motes are
      // anywhere in the same part of their swing. That is the whole difference
      // between a hundred leaves and one sheet with leaves drawn on it.
      // Derived from hashes this mote already has rather than drawing two more.
      // The first cut cost a pair of hash() calls per candidate — three lanes by
      // three layers is eighteen extra per fragment, and it took blooming to the
      // top of perf.mjs at 1.64x. seed and ph2 are already in hand and already
      // per-mote; re-combining them at different rates is free and decorrelates
      // far enough for an orbit nobody is going to check against the flutter.
      float mp = ph2 * 1.7 + seed.y * 3.1;
      float mrate = 0.55 + seed.y * 0.90;
      float swirl = t * mrate * 0.55 + fall * 0.60 + mp;
      // Wide across, shallow down: gravity is not negotiable, and a leaf that
      // looped as far vertically as horizontally would read as weightless. The
      // across amplitude stays inside the +/-1 lane the neighbourhood scans,
      // and the down amplitude inside the margin left by the seeded row offset.
      //
      // BOTH AMPLITUDES ARE BOUNDED BY THE NEIGHBOURHOOD, and the vertical one
      // is the tight one. Across, the k loop scans a lane either side, so half a
      // cell of travel is safe. Down, there is no loop at all — only this row —
      // so the mote's centre plus its own half-height has to stay inside the
      // cell or a leaf gets a straight edge sliced off it. At a half-height of
      // about 0.19 cells the seed range and the orbit together must not exceed
      // 0.78, which is what these two numbers add up to.
      // THE FLUTTER, and this is what "the petals STILL look rigid" is about.
      //
      // A petal does not spin. A propeller spins — constant angular velocity,
      // which is exactly what t * rate produces and exactly what was here. What
      // a petal does is FLUTTER: it glides on one face, stalls, tips over, and
      // glides back the other way. So its attitude is not a ramp and not a
      // sine; it DWELLS near two glide angles and passes quickly between them.
      // Raising |sin| to a low power gives that shape in one instruction — flat
      // near the extremes, steep through the middle.
      float sw = sin(swirl);
      float att = (sw < 0.0 ? -1.0 : 1.0) * pow(abs(sw), 0.40);
      //
      // And every other part of the motion is DERIVED from that attitude rather
      // than run on its own clock, because on a real petal they are one event.
      // It slides in the direction it is tilted (glide), so across is att. It
      // stalls while gliding and drops through the flip, so down runs at twice
      // the rate and is lowest exactly where att crosses zero. Two clocks that
      // merely happened to be near each other is what made the old motion read
      // as a mechanism; one clock driving all of it is what makes it read as a
      // thing falling through air.
      vec2 orbit = vec2(att, -cos(swirl * 2.0))
                 * vec2(0.42, 0.075) * mix(0.62, 1.0, leaf);
      vec2 centre = c + vec2(0.5 + flutter + orbit.x,
                             mix(0.32, 0.68, seed.y) + orbit.y);
      ...
      // AND IT TURNS WHILE IT SWINGS. Tying part of the spin to the orbit's own
      // phase couples attitude to path — the mote presents its face at the top
      // of a swing and its edge at the bottom, which is the read the owner is
      // missing when they say the fall looks weighty. Physics, as they put it:
      // a falling leaf rotates BECAUSE it is sliding through air, so the two
      // cannot be independent clocks.
      // The sprite ROCKS with the attitude instead of revolving. A slow linear
      // term stays — a petal does also turn lazily about its own vertical, and
      // without it the whole thing is perfectly periodic — but it is a tenth of
      // what it was: the rocking is the motion now, not a modulation on a spin.
      // (Rates are constants and per-cell hashes only, per the fall above.)
      float spin = att * 0.95 + ph2 + t * (0.10 + hash(c + 5.0) * 0.20);
      // The foreshortening IS the attitude, not a fourth clock. Face-on while
      // it glides, edge-on as it goes over — a petal really does vanish to very
      // nearly a line at the flip, and watching that happen is most of what
      // tells the eye it is a thin sheet rather than a lozenge.
      float tumble = att;
      ...
      q.x /= mix(0.26 + abs(tumble) * 0.74, 0.70 + abs(tumble) * 0.30, flower);
      float rib, eye;
  ...
  // The room reaches the motes as LIGHT, not as displacement: a working phrase
  // brightens what is falling. Where it reaches them as motion, it does so
  // through the flow clock in the phases above.
  return clamp(v, 0.0, 1.0) * w * (0.85 + drive * 0.25 + kick * 0.35);
}
...
float mFlame(vec2 uv, float t, float flow, float drive, float kick, out float core) {
  ...
  // THE WHIP. "Fire kind of looks like a tail now? With a long whip at the
  // end." Two things were compounding at the same place. The travelling wave's
  // amplitude went as h*h, so its largest excursion landed at the very tip —
  // and the tip is where the profile has tapered the flame down to a thread. A
  // thread displaced hard sideways is a whip; there is no other reading of it.
  //
  // The wave now peaks around three quarters of the way up and eases off over
  // the last quarter, which is also what a real flame does: the tip is where a
  // flame is coming APART, not where it is being cracked like a rope.
  float lean = (slow - 0.5) * 0.22 * h + wave * amp * h * (1.30 - 0.62 * h);

  // TWO EDGES, TORN SEPARATELY — and this is the seam.
  //
  // The silhouette was smoothstep(wid, .., abs(p.x - lean)), which is exactly
  // symmetric about the flame's axis, and then the erosion multiplied the whole
  // body by a field that is itself nearly symmetric about it. Everything that
  // happened on the left therefore happened on the right at the same height, and
  // a mirror line down the middle of a moving object is the most visible
  // artifact there is: the owner's "definite seam on the flame". Nothing was
  // drawing a line; the line is where the two halves meet.
  //
  // So each side gets its own tear function, sampled from the same rising field
  // at two unrelated offsets. The flame is no longer symmetric about anything.
  // Both drift in their SECOND axis as well as scrolling in the first. A field
  // sampled with a constant second argument is a fixed one-dimensional shape
  // sliding past — the exact fault the aurora's hem and rays had, and it reads
  // as a pattern being carried rather than as anything happening. Here it would
  // mean the same notches climbing the flame over and over. The drift lets a
  // tongue open and close where it stands, which is what tearing is.
  float tearR = fbm(vec2(p.y * 3.4 - rise * 1.5, 21.0 + t * 0.23));
  float tearL = fbm(vec2(p.y * 3.4 - rise * 1.5, 57.0 - t * 0.19));
  // Torn more with height, so tongues separate as they climb — but the seat
  // gets some of it too, or the base is a slab.
  float bite = 0.34 + h * 0.42;
  float halfR = wid * (0.32 + 1.05 * smoothstep(0.30, 0.72, tearR + (1.0 - h) * bite));
  float halfL = wid * (0.32 + 1.05 * smoothstep(0.30, 0.72, tearL + (1.0 - h) * bite));
  float e = p.x - lean;
  // EROSION BELONGS TO THE OUTLINE, NOT TO THE BODY. The old line multiplied the
  // whole flame by a thresholded noise field, which punches holes THROUGH it
  // wherever the field dips — the owner's other note, that "the darkness 'bites
  // chunks' out of the flame rather than the flame dancing in the darkness".
  // That is precisely what a coverage mask over an interior does. Fire tears at
  // its edges and stays lit in the middle, so the noise moves the boundary in
  // and out and the inside is left alone.
  float halfw = max(e > 0.0 ? halfR : halfL, 1e-4);
  float body = smoothstep(halfw, halfw * 0.15, abs(e));
  ...
  // The interior still has structure — it just varies in BRIGHTNESS now rather
  // than in existence, so the flame has depth without ever being holed.
  body *= 0.70 + 0.30 * smoothstep(0.28, 0.72, fast + (1.0 - h) * 0.30);
  // ...EXCEPT AT THE VERY TOP, where coming apart is the correct behaviour and
  // a continuous taper is not. Below three fifths this does nothing at all, so
  // the darkness still cannot bite chunks out of the body; over the last part
...
// A CYCLING spectrum, which is the whole difference between interference and a
// rainbow. A bow disperses white light once: red on one side, violet on the
// other, and that is the end of it. A thin film disperses it many times over —
// each extra half-wavelength of thickness puts the sequence round again — so
// what you see is the spectrum REPEATING in nested bands, and the bands are
// pastel because every order is overlapping the ones either side of it.
//
// Three cosines a third of a turn apart. Cheap, exactly periodic (so there is
...
vec3 filmSpectrum(float x, float sat) {
  ...
  vec3 c = 0.5 + 0.5 * cos(6.2831853 * x + vec3(0.0, 2.094, 4.188));
  // AND NOT A FALSE-COLOUR RAMP, which is what the last version was and what
  // the owner named exactly: "a sort of 'thermal vision' look... I want pretty,
  // striking rainbow light, not spy goggles."
  //
  // The tell is not the boldness — they asked for bold, twice. It is that a
  // cosine hue wheel at CONSTANT saturation and CONSTANT brightness is the
  // definition of a false-colour palette: every value in the field maps to a
  // different hue at the same luminance, so the picture carries its information
  // purely as hue, which is how a thermal camera draws and how nothing in the
  // sky ever looks. Real film colour is light, and light has bright parts and
  // dim parts.
  //
  // Two changes give it back. Saturation VARIES ALONG THE SWEEP, so the sequence
  // runs pale-vivid-pale and the pale zones let the cloud beneath show through;
  // the vivid bands then read as colour ON something rather than as a recolour
  // of everything. And the whole ramp is lifted toward white and scaled by its
  // own peak channel, so the bands differ in BRIGHTNESS as well as in hue —
  // which is the difference between light and a legend.
  float pale = 0.34 + 0.66 * (0.5 - 0.5 * cos(12.5663706 * x));
  vec3 tinted = mix(vec3(0.96, 0.97, 0.99), c, clamp(sat * pale, 0.0, 1.0));
  return tinted * (0.82 + 0.38 * max(max(c.r, c.g), c.b));
}

// IRIDESCENCE — cloud iridescence in the sky, an oil sheen on the wet ground.
//
// The owner's own redirection, and it is a better idea than what it replaces:
// "the rainbowy bits are just a kind of marbling that appears? Not even
// rainbowy or iridescent at all... it might be fun to play with cloud
// iridescence as well as iridescent puddles."
//
// The two previous attempts were both the wrong PHYSICS, which is why neither
// could be tuned into the right picture. First an arc: one large piece of fixed
// geometry, in the same place every time, so the only thing it could do was get
// brighter and dimmer. Then caustic filaments: a ridged noise field coloured by
// distance across each ridge — that is marbling, exactly as reported, because a
// filament's colour was a function of its own cross-section and the eye reads
// a set of independently coloured threads as veined stone.
//
// Interference is neither. Its structure is CONTOURS OF A THICKNESS FIELD, and
// the colour is a periodic function of that thickness. Get those two right and
// the picture arrives on its own: nested bands hugging the shape of whatever is
// diffracting, sliding as the shape changes, going round the spectrum again
// wherever the film is a little deeper.
//
//   The sky. The film is the CLOUD's own density — the same field that draws
//   the cloud, so the colour belongs to the cloud you can see rather than to a
//   pattern laid over it. It lives in a band around the cloud's own edge
//   threshold, because that is where the layer is thin and the droplets are all
//   one size; deep inside, the light has scattered too many times, and outside
//   there is nothing to diffract. Every reference the owner sent is a fringe
//   following an outline, and that is what an edge band draws.
//
//   The ground. Standing water with a film on it, in the ground plane's own
//   perspective so the swirls foreshorten toward the horizon instead of lying
//   flat on the picture. Patchy, because not all of the ground is wet.
//
...
vec3 mIridescence(vec2 uv, float t, float flow, float dens, vec2 thr, float body,
                  float drive, float kick, float pitch, float groundY, out float amt) {
  const vec2 SUN = vec2(-0.46, 0.30);
  float nearSun = smoothstep(1.15, 0.16, length(uv - SUN));

  // --- cloud iridescence ---------------------------------------------------
  // Thickness runs several orders across the fringe, so the bands nest. The
  // cloud's own body ramp is the bulk of it (0 to 1 over the edge, wherever
  ...
  // FEWER ORDERS. At 1.55 plus 0.55 of noise the fringe carried two or three
  // complete trips round the spectrum, and nested closed contours of hue are a
  // contour map — the other half of the thermal look. About one sweep across
  // the band is a rainbow; three is a topography of one.
  float film = across * 0.85
             + fbm(vec2(uv.x * 2.2 + flow * 0.22, uv.y * 2.2 - t * 0.030)) * 0.32;
  float skyAmt = fringe * (0.18 + nearSun * 0.70);

  // --- the puddles ---------------------------------------------------------
  // The ground line the drips already land on, PASSED IN rather than written
  // out again here. It used to be a second copy of the constant, and when main
  // lowered the ground the copy stayed where it was: the puddles then began
  // 0.14 above the water, so a band of iridescent patches hung in the air
  // between the skyline and the ground. That is most of what the owner saw as
  // clouds sitting "on the ground" — they were not clouds at all. One ground
  // line, one source, or this happens again the next time the horizon moves.
  float dep = max(groundY - uv.y, 0.0);
...
float mRidge(vec2 uv, float t, float flow, float drive,
  ...
  for (int i = 0; i < 3; i++) {
    ...
    // ...unless the air is HOT, in which case it runs the other way and that is
    // most of why desert read as temperate. Aerial perspective pales the
    // distance in any weather, but sand a few metres away in full sun is the
    // brightest thing in the frame, not the darkest — a near dune rendered
    // nearly black is a dune at dusk, which is precisely the cool, even,
    // late-afternoon light the owner is reacting to. At u_heat = 0 this is
    // unchanged, because a mountain range at nightfall really does go black in
    // front; at 1 the near ground comes up past the far.
    float shade = mix(0.46 - fi * 0.17, 0.50 + fi * 0.065, u_heat);
    v = mix(v, shade, below);
...
float mCone(vec2 uv, out float sky, out float down, out float descent,
  ...
  float ax = abs(uv.x);
  // A PEAK, and this is the "big scoop out of the top".
  //
  // The profile held CONE_RIM flat across the whole of |x| < 0.28 — a summit
  // plateau more than a quarter of the frame wide — and only then began to
  // descend. That is a mesa, and every attempt to make the middle of it read as
  // a crater full of lava was fighting the silhouette rather than fixing it:
  // a flat top with a dark centre reads as a bite taken out of the mountain
  // whatever is painted inside it. The owner has now said so three times, most
  // recently "it shouldn't just look like a chunk is off the top", and added
  // the answer: "Arenal here has a bit more of a peak, too."
  //
  // So the descent starts at the axis and the mountain comes to a point. The
  // exponent is just under one, which gives the slightly convex shoulder of a
  // stratovolcano — straight-ish flanks that round over into the summit —
  // rather than the dead-straight sides of a drawn triangle.
  //
  // NARROWER, too. The old flanks reached |x| = 1.02, so they left the frame
  // through the bottom corners and the mountain had no outside; that is most of
...
vec2 fkBodyFrame(vec2 p, float t){
  ...
  // A CLENCH IS A CROWDING, NOT A ZOOM. This scaled the body frame by up to
  // 34% on an onset — and since the frame carries the outline AND the grain,
  // every beat resized the entire flock. That is a camera moving toward the
  // birds, not birds drawing together, and it is what the owner means by "it
  // does seem to shrink and throb a bit in a way that kind of takes you out of
  // it". A zoom is the one motion in this mood that has no bird in it.
  //
  // So the geometry barely moves and the startle goes into DENSITY instead
  // (see the thick term at the end of mFlock): the silhouette holds its size
  // while the birds inside it pack tighter and the mass darkens. That is what a
  // murmuration does when something startles it.
  float ball = 1.0 + fkClench * 0.09;
...
float mFlock(vec2 uv, float t, float drive, float kick) {
  ...
  // The startle now lives here almost entirely, since fkBodyFrame stopped
  // resizing the flock to do it: the mass darkens and packs where it used to
  // zoom. Same event, read as birds instead of as a lens.
  thick *= 1.0 + fkTurn(t) * 0.35 + fkClench * 0.62;
...
vec3 mWisps(vec2 uv, float t, float w, float flow, float drive) {
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      ...
      // The floor comes up too: a wisp that dims to 0.3 of itself in a mood this
      // dark has gone out, and a light that goes out is not a light that hangs.
      float breath = (0.48 + 0.52 * (0.5 + 0.5 * sin(t * (0.5 + h.x * 0.5) + h.y * 6.28)))
                   * (0.58 + drive * 0.9);
      ...
      // A LANTERN NEEDS A FLAME IN IT. The owner, after being told twice that
      // these were there: "I do see some faintly glowing blobs that mostly
      // blend in with the green fog, but surely that's not it?" That is it, and
      // that is the problem — a wide soft halo with a barely brighter middle is
      // indistinguishable from a patch of lit mist, which is the one thing this
      ...
      // a cold green sitting in a wood whose fog, canopy light and shafts are
      // all green: a green light on a green ground is a change of brightness,
      // and this mood's brightness is already busy. Cyan through to a cold
      // white-blue reads as OTHER — foxfire is famously not the colour of the
      // wood it grows on — and the rare gold ones now go warmer still, so the
      // two kinds are unmistakably not the mist.
      vec3 tint = mix(vec3(0.42, 0.98, 0.86), vec3(0.62, 0.86, 1.0), h.x);
      tint = mix(tint, vec3(1.0, 0.80, 0.34), step(0.86, hash(id * 7.3)) * 0.8);
      v += tint * body;
    }
  }
...
float mClouds(vec2 uv, float t, float flow, float drive, float horizon,
  ...
  // GENTLY. The offset was 0.16, which over the height of this sky is a divide
  // running 6.25 down to 0.96 — a better than six-to-one compression, and that
  // is not a landscape's sky, it is the view from an aeroplane. Everything it
  // drew came out stretched flat and stacked in layers toward the horizon,
  // which is precisely what the owner saw: "it makes it look like we're very
  // high up above the world", and stratospheric is the right word for the
  // shapes a divide that steep produces. At 0.40 the range is 2.5 to 0.78,
  // about three to one — enough that a far cloud is smaller than a near one,
  // not so much that the sky becomes a floor seen from above.
  float dz = 1.0 / (above + 0.40);
  vec2 q = vec2(uv.x * dz * 1.45 + flow * 0.35 + t * 0.01, dz * 1.15);
  // BILLOWS, which is what makes a cumulus fluffy rather than a blot. A cloud's
  // outline is lumpy at every scale, and the cheap way to get that is a domain
  // warp: displace the point where the field is sampled by a second, coarser
  // field, so the first one's contours are pushed in and out of themselves
  // instead of running smooth. One extra pair of octaves buys the cauliflower.
  // FLUFFIER STILL — "I'm thinking fluffier, less stratospheric looking", now
  // twice. A cumulus is lumpy at EVERY scale: big cauliflower heads with
  // smaller cauliflower on them, all convex, all bulging outward. One warp gave
  // the big heads and left their edges smooth between them, which still reads
  // as a soft blot. A second, finer warp puts the small bulges on, and the
  // cheap noise() is enough for it — at that scale it is the presence of the
  // lumps that reads, not their internal detail.
  vec2 warp = vec2(fbm(q * 1.7 + 11.0), fbm(q * 1.7 + 29.0)) - 0.5;
  vec2 warp2 = vec2(noise(q * 5.1 + 53.0), noise(q * 5.1 + 71.0)) - 0.5;
  float cl = fbm(q + warp * 0.72 + warp2 * 0.24);
  ...
  // has to be the cloud's own field rather than a second one laid over it — or
  // the colour would belong to a different cloud than the one you can see.
  dens = cl;
  ...
  // TIGHTER, because a fluffy cloud has an EDGE. The band was 0.18 wide, which
  // over a gently-sloping field is a very long fade — and a cloud that fades
  // out over a third of its own width is a smudge, whatever shape its contour
  // has. Halving the band is most of what turns the deck from vapour into
  // something with a top and a side.
  // ...and tighter again, because the edge is what says fluffy. A cloud has a
  // definite boundary with soft detail ON it; a wide coverage ramp gives soft
  // detail INSTEAD of a boundary, which is fog.
  thr = vec2(0.68 - drive * 0.38, 0.755 - drive * 0.24);
  float body = smoothstep(thr.x, thr.y, cl);
  // A CLOUD BASE, well above the skyline. Nothing below it, and nothing at the
  // skyline either: the deck's features fall below resolving distance there
  // long before the deck does, so it fades into haze rather than ending at a
  // line.
  //
  // The gate used to open 0.11 above the horizon, which is nearly nothing, so
  // the deck ran right down to the ground and the owner read the result as
  // clouds "close to the ground, even ON the ground sometimes". The divide is
  // honest — a deck really does meet the horizon at infinite distance — but
  // what a landscape in weather looks like is air underneath the cloud, and
  // the sky here is only 0.88 tall, so the far half of an honest deck is all
  // haze anyway. Give the bottom half of the sky back to the air: the clouds
  // are then at a height, and there is room to see them as a ceiling rather
  // than as scenery standing on the ground.
  body *= smoothstep(0.10, 0.42, above);
...
float mStars(vec2 uv, float t, float w, float strike, float meteors, out vec3 tint) {
  tint = vec3(1.0);
  vec2 p = uv * 26.0;
  vec2 i = floor(p), f = fract(p);
  vec2 o = hash2(i);
  ...
  float meteor = smoothstep(0.012, 0.0, side)
               ...
               * strike * strike * 1.6 * smoothstep(0.34, 0.04, age) * meteors;
...
float mAurora(vec2 uv, float t, float drive, float kick, float wake, out float high) {
  float x = uv.x * 1.3;
  ...
  // being moved.
  //
  // A CURTAIN, WHICH MEANS THE STRUCTURE HANGS FROM THE HEM AND TRAVELS ALONG
  // IT. Every field here used to be sampled against uv.y — the frame's own
  // vertical — so the folds and the rays were a pattern the curtain slid across
  // rather than a pattern the curtain HAD. Anchored to up (height inside the
  // curtain, measured from its own hem) they belong to the veil, and the whole
  // sheet then moves as one body. That is most of the difference between
  // something that bounces and something that hangs.
  float hem = -0.22 + fbm(vec2(x * 1.1 + t * 0.075, 3.7 + t * 0.045)) * 0.26
            // A wave running ALONG the hem, which is the motion everyone
            // recognises: the lower edge of a curtain ripples down its length.
            + sin(x * 3.1 - t * 0.62) * 0.020
            + sin(x * 6.7 + t * 0.41 + 1.3) * 0.008
            + kick * 0.05;
  float up = uv.y - hem;
  // The folds shear with height: the top of the sheet lags the bottom, so it
  // leans and unleans instead of sliding rigidly sideways.
  //
  // A LINEAR SHEAR, AND IT HAS TO BE LINEAR. This was two sinusoids of up,
  // amplitude 0.085 and 0.045 — and the rays below stand at a spacing of about
  // 1/9.5 in the same units, so the warp shoved each ray sideways by MORE THAN
  // A WHOLE RAY-WIDTH along its own length, with a full cycle inside the
  // curtain's height. The owner's note is exactly that: the vertical lines
  // "aren't so straight anymore. Something is warping them."
  //
  // The distinction is not about amount, it is about which functions preserve
  // the shape. A shear that is LINEAR in up maps straight lines to straight
  // lines — the sheet leans as one body, which is what a hanging curtain does.
  // A shear that is SINUSOIDAL in up bends every line it touches into an S. So
  // the lean is now linear and slow, and it is the HEM that snakes (fbm plus
  // the travelling waves above): the curtain's lower edge ripples along its
  // length while the shafts hanging from it stay shafts.
  float lean = sin(t * 0.21) * 0.10 + sin(t * 0.13 + 1.7) * 0.06;
  // A waver is still welcome, at an amplitude that is a tenth of the ray
  // spacing rather than ten times it: life, not distortion.
  float xf = x + up * lean + sin(up * 1.6 - t * 0.29) * 0.010;
  float fold = fbm(vec2(xf * 4.0 - t * 0.06, up * 0.9 + t * 0.11));
  ...
  float rays = smoothstep(0.30, 0.80, fbm(vec2(xf * 9.5 + t * 0.15, 11.0 + t * 0.085)));
  ...
  // How high the curtain climbs rides the charge, so a waking aurora GROWS up
  // the sky rather than fading in at full height. The ceiling and the shoulder
  // both rise with it and can never meet: the gap between them is
  // mix(0.06, 0.28, wake), positive at both ends.
  ...
  // THE GATE IS AN ENVELOPE, NOT A READING, and that is the whole fix.
  //
  // This has now been wrong in three different ways, and they are worth keeping
  // written down because each one looked like a tuning problem and none was.
  // First the gate was thrown wide open (a floor of 0.3 under a 0.32-0.58 ramp),
  // which made night an aurora theme with a sky behind it. Then it was closed to
  // 0.54-0.76, which put it outside what the centroid can reach, and the motif
  // simply never drew. Then it was split into a low gate and a high one, which
  // put a permanent arc on the horizon the owner had not asked for AND left the
  // curtain tracking the register note by note: "it sort of... bounced? And then
  // disappeared entirely, only to reappear (and fade in and out) later."
  //
  // Every one of those is the same mistake at a different setting: reading a
  // LIVE FEATURE as the answer, when the answer wanted a state. An aurora is a
  // state. It takes time to rise, and it outlasts what woke it — a substorm
  // does not stop when the sun does.
  //
  // So wake arrives already charged (see WAKE_RISE_TAU / WAKE_FALL_TAU in
  // this file): the top of the register charges it over a couple of seconds and
  // it drains over half a minute. Nothing here reads the register directly.
  // Below the gate there is no curtain at all and the sky is the mood; above it
  // the curtain rises, climbs, and then LINGERS, which is what was asked for.
  float v = body * (0.3 + smoothstep(0.35, 0.75, fold) * 0.7)
          * (0.32 + rays * 0.9);
  // The hem burns brightest — the lower edge is where the excited band is
  // densest, and it is the one feature everybody recognises.
  v += edge * 0.85;
  ...
  return v * wake * (0.5 + drive * 0.9 + kick * 0.5);
}
...
void main() {
  ...
  if (W_cone > 0.0) {
    ...
    //
    // STATIC RELIEF ON THE ROCK, and this is the answer to "it's as if the
    // crags are oozing down along with the lava".
    //
    // Nothing was drawing the crags at all: the cone was filled with a flat
    // 0.04 and the only structure anywhere on it was the lava field, which
    // scrolls. So the dark shapes BETWEEN the streams were not rock — they were
    // the gaps between moving things, and gaps between moving things move. With
    // no fixed feature anywhere on the mountain the eye had nothing to hold on
    // to and read the whole surface as flowing.
    //
    // The cure is to give the rock its own visible, permanently still texture:
    // ridges and gullies lit from the same upper-left the rest of the engine
    // uses, derived from the profile roughness that was already being computed
    // and thrown away. Once the mountain has fixed relief, the lava is plainly
    // running OVER something, and the something is not going anywhere.
    float gully = (lnoise(vec2(uv.x * 13.0, uv.y * 4.5 + 31.0)) - 0.5)
                + (lnoise(vec2(uv.x * 31.0, uv.y * 9.0 + 7.0)) - 0.5) * 0.5;
    // Lit across the slope: a gully's far wall faces the light and its near
    // wall does not, which is what makes rock read as carved rather than
    // painted. Fading out toward the summit, where the profile is smoother.
    float relief = gully * smoothstep(0.02, 0.34, cdown) * 0.9;
    g = mix(g, clamp(0.035 + relief * 0.07, 0.012, 0.16), rock * W_cone);
  ...
  if (W_crystals > 0.0) {
    ...
    if (W_tunnel > 0.0) {
      // THE LAMP ITSELF ANSWERS THE REGISTER, not only the quartz it finds.
      // Brightening the crystals alone gives a lit seam in an equally-lit
      // passage, and the owner's two pictures are of the whole CAVE: "dark,
      // dank cavern vs. crystal grotto". So the light the pool carries is
      // small and local down at the bottom of the keyboard — a guttering
      // thing that shows a patch of wet rock — and floods the passage at the
      // top of it. Squared, so the middle of the instrument is still dim.
      float reg = clamp(u_centroid, 0.0, 1.0);
      float lamp = 0.28 + reg * reg * 1.55;
      float seen = clamp(pool * lamp, 0.0, 1.0);
      g *= mix(0.05, 1.0, seen);
      ...
      lift += seen * seen * (0.15 + reg * 0.45) * W_tunnel;
      spec += seen * seen * (0.11 + reg * 0.38) * W_tunnel * (0.5 + u_sparkle * 0.8);
    }
  }
  if (W_snow > 0.0) {
  ...
  if (W_stars > 0.0) {
    vec3 stint;
    float s = mStars(uv, u_t, W_stars, u_pulse, u_meteors, stint);
  ...
  if (W_ripples > 0.0) {
    ...
    // HEAT SHIMMER, and it is the one cue that cannot be faked with colour.
    // The owner's note is that desert "seems pretty temperate", and the reason
    // is that everything saying "hot" was a palette choice — warm sand under a
    // warm sky is equally a photograph of a beach in April. What says heat is
    // that the air itself has stopped being transparent: a refractive-index
    // gradient over baking ground, so the ground SWIMS.
    //
    // Vertical displacement only, because the layers of hot air are horizontal,
    // and strongest just under the horizon where the line of sight grazes the
    // surface for the longest distance. Near sand, seen through a metre of air,
    // is steady — which is exactly the gradient that makes it read as distance
    // rather than as a wobbly filter over the whole picture.
    vec2 ruv = uv;
    if (u_heat > 0.0) {
      float dep = max(ridgeHorizonY - uv.y, 0.0);
      float swim = smoothstep(0.32, 0.005, dep) * u_heat;
      ruv.y += ((fbm(vec2(uv.x * 6.5, u_t * 1.7)) - 0.5) * 0.020
             + sin(uv.x * 21.0 + u_t * 2.7) * 0.0026) * swim;
    }
    float rip = mRipples(ruv, u_t, u_flow, u_rms, ridgeLayer, ridgeHorizonY) * W_ripples;
  ...
  float aurHigh = 0.0;
  if (W_aurora > 0.0) {
    aur = mAurora(uv, u_t, u_rms, u_pulse, u_wake, aurHigh) * W_aurora;
  }
  if (W_facets > 0.0) {
    float seam, flare;
  ...
  // LIT. At 0.5 toward a mid palette step the deck was barely present, and in a
  // dark sky that is the rest of why the iridescence read as false colour: the
  // bow is additive, so if there is no lit cloud under it the colour IS the
  // cloud, and a shape made entirely of hue is a thermal blob by construction.
  // Give the deck its own light and the bands have something to be bands ON.
  col = mix(col, mix(u_c2, u_c4, 0.62), clamp(cloud, 0.0, 1.0) * 0.74);
  col += mix(u_c3, u_c4, 0.35) * clamp(cloudRim, 0.0, 1.0) * 0.85;
  // THE GLOW OVER THE VENT. Every one of the reference photographs has it and
  // this had none: an erupting summit throws enough light to stain the air
  // above it, and that column of dull red in the night sky is a good part of
  // what makes the picture read as ferocious rather than as a lit model. It is
  // also the only thing in the mood that says the mountain is producing the
  // light rather than being lit by something off-frame.
  //
  // Anchored to the summit and fenced to the sky, so it sits above the skyline
  // and never washes over the rock. Rises with the room, because the eruption
  // working harder is exactly what should brighten it.
  if (W_cone > 0.0 && W_lava > 0.0) {
    // TIGHTER, so it is a glow ON the summit rather than a wash over the sky.
    // The owner liked it and worried about it in the same breath — "the orange
    // glow behind it is pretty but I wonder if we might be losing some drama" —
    // and they are right about the mechanism: a night mood's drama is the
    // difference between the fire and the dark, so anything that lifts the
    // whole sky spends the contrast the cone is silhouetted by. Half the
    // radius, and redder, keeps the incandescence and gives the black back.
    vec2 vd = (uv - vec2(0.0, CONE_RIM)) / vec2(0.19, 0.24);
    float halo = exp(-dot(vd, vd)) * skyMask;
    col += vec3(1.0, 0.26, 0.05) * halo * W_lava
         * (0.14 + u_rms * 0.26 + u_pulse * 0.30);
  }
  ...
  if (W_rainbow > 0.0) {
    ...
    vec3 bow = mIridescence(uv, u_t, u_flow, cloudDens, cloudThr, cloudBody,
                            u_rms, u_pulse, u_centroid, groundY, bowAmt);
    // Trimmed, because two things under it got brighter in the same pass: the
    // spectrum now peaks near 1.2 rather than 1.0 (it is scaled by its own peak
    // channel, which is what stops it reading as a flat legend), and the deck it
    // sits on is lit at 0.74 instead of 0.5. Left at 1.25 the two together run
    // the fringe past 1.5 and blow the cloud edge to white — which loses the
    // colour, and the colour is the entire motif.
    col += bow * bowAmt * W_rainbow * 0.95;
  }
  ...
  if (W_lava > 0.0) {
    // LIVID RED-ORANGE, per the references. The ramp used to finish at a pale
    // straw white, which is the colour of a foundry pour seen close up and not
    // the colour of a mountain at night — in the Arenal photographs even the
    // freshest rock is a fierce orange and the white is only ever in the vent
    // itself. Pulling the top of the ramp back from straw to hot orange also
    // buys contrast against the black cone, which is where this mood's drama
    // lives: "make sure the contrast with the background is striking."
    vec3 crust = vec3(0.48, 0.06, 0.015);
    vec3 run = vec3(1.0, 0.30, 0.035);
    vec3 fresh = vec3(1.0, 0.68, 0.22);
  ...
  if (W_petals > 0.0) {
    ...
    // AUTUMN'S COLOUR BELONGS TO THE LEAVES, not to the light — the owner's
    // note, and they are right about the season: an October wood is lit by a
    // thin grey-green daylight, and every bit of the copper in it is hanging on
    // a branch or lying on the ground. Tinting the whole frame amber was doing
    // the work of saying "autumn" with one flat gesture, which is the same
    // mistake barren's monochrome made.
    //
    // So the leaves carry their own ramp, off the palette entirely, the way
    // fire and the aurora already do: oxblood through rust to a dry gold, spread
    // by the same per-mote hash. The palette is then free to be the LIGHT, and
    // it has gone quiet and cool to match.
    vec3 fallen = mix(vec3(0.38, 0.08, 0.03), vec3(0.78, 0.26, 0.05),
                      smoothstep(0.0, 0.55, petalTint));
    fallen = mix(fallen, vec3(0.93, 0.63, 0.18), smoothstep(0.45, 1.0, petalTint));
    pc = mix(pc, fallen, u_leaf);
  ...
  // THE HAZE THAT GOES WITH THE SHIMMER. Hot air over a desert is full of dust
  // and its own turbulence, and both scatter — so contrast collapses toward the
  // horizon and the band just above it bleaches out almost to white. That
  // washed-out, high-key look is half of what a photograph of real heat has and
  // a temperate landscape never does, and it costs one mix.
  if (u_heat > 0.0) {
    float band = smoothstep(0.42, 0.0, abs(uv.y - ridgeHorizonY - 0.03));
    col = mix(col, mix(u_c4, vec3(1.0), 0.55), band * u_heat * 0.34);
  }
  ...
  col += clamp(wisp, vec3(0.0), vec3(1.5)) * 1.35;
...
// THE AURORA'S WAKE, and it is a third timescale again — slower than weather
// on the way down and quicker on the way up.
//
// A gate read straight off the centroid makes the curtain track the register
// note for note: it appears, vanishes, reappears, fades in and out. The owner
// watched exactly that and called it bouncing. An aurora does not bounce. It
// takes a while to rise, and once it is up it STAYS up for a long time after
// the thing that woke it has passed — the substorm outlives the cause.
//
// So the register does not set the curtain's brightness; it charges an
// envelope, and the envelope sets the brightness. Rise is a couple of seconds
// (a curtain kindling, not a switch), fall is most of a minute, which is what
// "it lingers while that register is present" asks for and rather more besides:
// play high once and the sky answers for a long time afterwards. It is also
// what makes the motif survive real playing, where a passage in the top octave
// is a handful of seconds at a time rather than a sustained state.
const WAKE_RISE_TAU = 2.2;
const WAKE_FALL_TAU = 26.0;

// One per renderer instance; seeded to IDLE's values so the first frame after
// a theme load does not lurch in from zero.
// GLSL's smoothstep, for the handful of gates that have to agree between the
// CPU and the shader.
...
function createMotionSmoother() {
  const v = { warp: 0, shift: 0.4, light: 0, weather: 0, wake: 0 };
  return (f, dt) => {
    ...
    // Read off the RAW centroid rather than v.shift: the wake wants to notice a
    // brief visit to the top of the instrument, and v.shift's one-second
    // follower rounds a short one off before it is ever tested. The envelope
    // below supplies all the smoothing this needs, and far more of it.
    //
    // The gate is the top of the register and nothing below it — 0.66 to 0.86.
    // "Invisible, or nearly so, until the very high register" is a specification
    // and this is it written down; there is deliberately no low gate any more,
    // because the pass that added one was answering a different sentence.
    const wakeTarget = smooth01(0.66, 0.86, f.centroid);
    const kTau = wakeTarget > v.wake ? WAKE_RISE_TAU : WAKE_FALL_TAU;
    v.wake += (wakeTarget - v.wake) * (1 - Math.exp(-dt / kTau));
...
const UNIFORM_NAMES = [
  ...
  'u_flow', 'u_cur', 'u_centroid', 'u_angular', 'u_canopy', 'u_leaf', 'u_bark', 'u_meteors', 'u_wake',
  'u_flower', 'u_heat',
  'u_mw[0]',
];
...
function createGL(canvas, reducedMotion) {
  ...
  function build() {
    ...
    // REPORTED, because losing it is invisible and expensive. Without this
    // extension the flock falls back to a three-tap forward difference — three
    // full traces per fragment instead of one — which is exactly the cost that
    // made it the dearest mood in the set before dFdx replaced it. A context
    // restored onto a degraded or software renderer (say, after the GPU process
    // has gone down) can come back without the extension, and the only symptom
    // is that one mood starts stuttering again for no visible reason. Now it
    // says so, and tools and the HUD can both read it.
    deriv = hasDerivatives;
  ...
  function frame(t, dt, f, intensity) {
    ...
    gl.uniform1f(U.u_flower, th.params.flower || 0);
    gl.uniform1f(U.u_heat, th.params.heat || 0);
    gl.uniform1f(U.u_meteors, th.params.meteors === undefined ? 1 : th.params.meteors);
  ...
  return {
    resize, setSize, setTheme, frame, kind: 'webgl',
    get deriv() { return deriv; },
  };
}
